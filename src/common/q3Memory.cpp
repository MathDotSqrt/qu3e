...
//--------------------------------------------------------------------------------------------------
void q3Stack::Reserve( q3u32 size )
{
	assert( !m_index );
	...
	{
		if ( m_memory ) q3Free( m_memory );
		m_memory = (q3u8*)q3Alloc( size );
		m_stackSize = size;
	}
}

//--------------------------------------------------------------------------------------------------
void *q3Stack::Allocate( q3i32 size )
{
...
//--------------------------------------------------------------------------------------------------
void *q3Heap::Allocate( q3i32 size )
{
	q3i32 sizeNeeded = size + sizeof( q3Header );
	q3FreeBlock* firstFit = NULL;

	for ( q3i32 i = 0; i < m_freeBlockCount; ++i )
	{
...
{
	assert( memory );
	q3Header* node = (q3Header*)Q3_PTR_ADD( memory, -q3i32( sizeof( q3Header ) ) );

	q3Header* next = node->next;
	q3Header* prev = node->prev;
	q3FreeBlock* nextBlock = NULL;
	q3i32 prevBlockIndex = ~0;
	q3FreeBlock* prevBlock = NULL;
	q3i32 freeBlockCount = m_freeBlockCount;

	for ( q3i32 i = 0; i < freeBlockCount; ++i )
	{
		q3FreeBlock* block = m_freeBlocks + i;
	...
	if ( !merged )
	{
		...
		if ( m_freeBlockCount == m_freeBlockCapacity )
		{
			...
			q3i32 oldCapacity = m_freeBlockCapacity;
...
// q3PagedAllocator
//--------------------------------------------------------------------------------------------------
q3PagedAllocator::q3PagedAllocator( q3i32 elementSize, q3i32 elementsPerPage )
{
...
void* q3PagedAllocator::Allocate( )
{
	...
	else
	{
		...
		q3i32 blocksPerPageMinusOne = m_blocksPerPage - 1;
		for ( q3i32 i = 0; i < blocksPerPageMinusOne; ++i )
		{
