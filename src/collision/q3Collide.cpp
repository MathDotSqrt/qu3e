...
// qBoxtoBox
//--------------------------------------------------------------------------------------------------
inline bool q3TrackFaceAxis( q3i32* axis, q3i32 n, q3r32 s, q3r32* sMax, const q3Vec3& normal, q3Vec3* axisNormal )
{
	if ( s > q3r32( 0.0 ) )
		return true;
...
//--------------------------------------------------------------------------------------------------
inline bool q3TrackEdgeAxis( q3i32* axis, q3i32 n, q3r32 s, q3r32* sMax, const q3Vec3& normal, q3Vec3* axisNormal )
{
	if ( s > q3r32( 0.0 ) )
		return true;

	q3r32 l = q3r32( 1.0 ) / q3Length( normal );
	s *= l;
...
//--------------------------------------------------------------------------------------------------
void q3ComputeReferenceEdgesAndBasis( const q3Vec3& eR, const q3Transform& rtx, q3Vec3 n, q3i32 axis, q3u8* out, q3Mat3* basis, q3Vec3* e )
{
...
#define On( a ) \
	((a) < q3r32( 0.005 ) && (a) > -q3r32( 0.005 ))

q3i32 q3Orthographic( q3r32 sign, q3r32 e, q3i32 axis, q3i32 clipEdge, q3ClipVertex* in, q3i32 inCount, q3ClipVertex* out )
{
	q3i32 outCount = 0;
	q3ClipVertex a = in[ inCount - 1 ];

	for ( q3i32 i = 0; i < inCount; ++i )
	{
		q3ClipVertex b = in[ i ];

		q3r32 da = sign * a.v[ axis ] - e;
		q3r32 db = sign * b.v[ axis ] - e;
...
q3i32 q3Clip( const q3Vec3& rPos, const q3Vec3& e, q3u8* clipEdges, const q3Mat3& basis, q3ClipVertex* incident, q3ClipVertex* outVerts, q3r32* outDepths )
{
	q3i32 inCount = 4;
	q3i32 outCount;
	q3ClipVertex in[ 8 ];
	...
	outCount = q3Orthographic( q3r32( 1.0 ), e.x, 0, clipEdges[ 0 ], in, inCount, out );

	if ( !outCount )
		return 0;

	inCount = q3Orthographic( q3r32( 1.0 ), e.y, 1, clipEdges[ 1 ], out, outCount, in );

	if ( !inCount )
		return 0;

	outCount = q3Orthographic( q3r32( -1.0 ), e.x, 0, clipEdges[ 2 ], in, inCount, out );

	if ( !outCount )
		return 0;

	inCount = q3Orthographic( q3r32( -1.0 ), e.y, 1, clipEdges[ 3 ], out, outCount, in );

	// Keep incident vertices behind the reference face
	outCount = 0;
	for ( q3i32 i = 0; i < inCount; ++i )
	{
		q3r32 d = in[ i ].v.z - e.z;
...
void q3SupportEdge( const q3Transform& tx, const q3Vec3& e, q3Vec3 n, q3Vec3* aOut, q3Vec3* bOut )
{
	...
	q3r32 signx = q3Sign( n.x );
	q3r32 signy = q3Sign( n.y );
	q3r32 signz = q3Sign( n.z );
...
void q3BoxtoBox( q3Manifold* m, q3Box* a, q3Box* b )
{
	...
	q3Mat3 absC;
	bool parallel = false;
	const q3r32 kCosTol = q3r32( 1.0e-6 );
	for ( q3i32 i = 0; i < 3; ++i )
	{
		for ( q3i32 j = 0; j < 3; ++j )
		{
			q3r32 val = q3Abs( C[ i ][ j ] );
			absC[ i ][ j ] = val;

			if ( val + kCosTol >= q3r32( 1.0 ) )
				parallel = true;
		}
	}
	...
	// Query states
	q3r32 s;
	q3r32 aMax = -Q3_R32_MAX;
	q3r32 bMax = -Q3_R32_MAX;
	q3r32 eMax = -Q3_R32_MAX;
	q3i32 aAxis = ~0;
	q3i32 bAxis = ~0;
	q3i32 eAxis = ~0;
	q3Vec3 nA;
	...
	if ( !parallel )
	{
		...
		if ( q3TrackEdgeAxis( &eAxis, 6, s, &eMax, q3Vec3( q3r32( 0.0 ), -C[ 0 ][ 2 ], C[ 0 ][ 1 ] ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 7, s, &eMax, q3Vec3( q3r32( 0.0 ), -C[ 1 ][ 2 ], C[ 1 ][ 1 ] ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 8, s, &eMax, q3Vec3( q3r32( 0.0 ), -C[ 2 ][ 2 ], C[ 2 ][ 1 ] ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 9, s, &eMax, q3Vec3( C[ 0 ][ 2 ], q3r32( 0.0 ), -C[ 0 ][ 0 ] ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 10, s, &eMax, q3Vec3( C[ 1 ][ 2 ], q3r32( 0.0 ), -C[ 1 ][ 0 ] ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 11, s, &eMax, q3Vec3( C[ 2 ][ 2 ], q3r32( 0.0 ), -C[ 2 ][ 0 ] ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 12, s, &eMax, q3Vec3( -C[ 0 ][ 1 ], C[ 0 ][ 0 ], q3r32( 0.0 ) ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 13, s, &eMax, q3Vec3( -C[ 1 ][ 1 ], C[ 1 ][ 0 ], q3r32( 0.0 ) ), &nE ) )
			return;
		...
		if ( q3TrackEdgeAxis( &eAxis, 14, s, &eMax, q3Vec3( -C[ 2 ][ 1 ], C[ 2 ][ 0 ], q3r32( 0.0 ) ), &nE ) )
			return;
	}

	// Artificial axis bias to improve frame coherence
	const q3r32 kRelTol = q3r32( 0.95 );
	const q3r32 kAbsTol = q3r32( 0.01 );
	q3i32 axis;
	q3r32 sMax;
	q3Vec3 n;
	q3r32 faceMax = q3Max( aMax, bMax );
	...
	if ( q3Dot( n, btx.position - atx.position ) < q3r32( 0.0 ) )
		n = -n;
