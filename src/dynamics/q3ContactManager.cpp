...
void q3ContactManager::TestCollisions( void )
{
	...
	while( constraint )
	{
		...
		for ( q3i32 i = 0; i < manifold->contactCount; ++i )
		{
			q3Contact *c = manifold->contacts + i;
			c->tangentImpulse[ 0 ] = c->tangentImpulse[ 1 ] = c->normalImpulse = q3r32( 0.0 );
			q3u8 oldWarmStart = c->warmStarted;
			c->warmStarted = q3u8( 0 );

			for ( q3i32 j = 0; j < oldManifold.contactCount; ++j )
			{
				...
				if ( c->fp.key == oc->fp.key )
				{
					...
					c->warmStarted = q3Max( oldWarmStart, q3u8( oldWarmStart + 1 ) );
					break;
				}
			}
		}
		...
		{
			q3i32 now_colliding = constraint->m_flags & q3ContactConstraint::eColliding;
			q3i32 was_colliding = constraint->m_flags & q3ContactConstraint::eWasColliding;
...
void q3ContactManager::RenderContacts( q3Render* render ) const
{
	...
	while ( contact )
	{
		...
		for ( q3i32 j = 0; j < m->contactCount; ++j)
		{
			const q3Contact *c = m->contacts + j;
			q3f32 blue = (q3f32)(255 - c->warmStarted) / 255.0f;
			q3f32 red = 1.0f - blue;
