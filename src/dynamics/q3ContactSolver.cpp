...
//--------------------------------------------------------------------------------------------------
void q3ContactSolver::PreSolve( q3r32 dt )
{
	for ( q3i32 i = 0; i < m_contactCount; ++i )
	{
		...
		for ( q3i32 j = 0; j < cs->contactCount; ++j )
		{
			...
			c->bias = -Q3_BAUMGARTE * (q3r32( 1.0 ) / dt) * q3Min( q3r32( 0.0 ), c->penetration + Q3_PENETRATION_SLOP );
			...
			q3r32 dv = q3Dot( vB + q3Cross( wB, c->rb ) - vA - q3Cross( wA, c->ra ), cs->normal );
	...
	for ( q3i32 i = 0; i < m_contactCount; ++i )
	{
		...
		for ( q3i32 j = 0; j < cs->contactCount; ++j )
		{
			...
			if ( m_enableFriction )
			{
				for ( q3i32 i = 0; i < 2; ++i )
				{
					q3r32 lambda = -q3Dot( dv, cs->tangentVectors[ i ] ) * c->tangentMass[ i ];

					// Calculate frictional impulse
					q3r32 maxLambda = cs->friction * c->normalImpulse;

					// Clamp frictional impulse
					q3r32 oldPT = c->tangentImpulse[ i ];
			...
			// Normal
			{
				...
				// Normal impulse
				q3r32 vn = q3Dot( dv, cs->normal );

				// Factor in positional bias to calculate impulse scalar j
				q3r32 lambda = c->normalMass * (-vn + c->bias);

				// Clamp impulse
				q3r32 tempPN = c->normalImpulse;
				c->normalImpulse = q3Max( tempPN + lambda, q3r32( 0.0 ) );
