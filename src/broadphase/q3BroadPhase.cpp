...
q3BroadPhase::q3BroadPhase( q3ContactManager *manager )
{
	...
	m_moveCount = 0;
	m_moveCapacity = 64;
	m_moveBuffer = (q3i32*)q3Alloc( m_moveCapacity * sizeof( q3i32 ) );
}
...
{
	q3i32 id = m_tree.Insert( aabb, box );
...
//--------------------------------------------------------------------------------------------------
void q3BroadPhase::Update( q3i32 id, const q3AABB& aabb )
{
...
//--------------------------------------------------------------------------------------------------
bool q3BroadPhase::TestOverlap( q3i32 A, q3i32 B ) const
{
	return q3AABBtoAABB( m_tree.GetFatAABB( A ), m_tree.GetFatAABB( B ) );
}

//--------------------------------------------------------------------------------------------------
void q3BroadPhase::BufferMove( q3i32 id )
{
	if ( m_moveCount == m_moveCapacity )
	{
		q3i32* oldBuffer = m_moveBuffer;
		m_moveCapacity *= 2;
		m_moveBuffer = (q3i32*)q3Alloc( m_moveCapacity * sizeof( q3i32 ) );
		memcpy( m_moveBuffer, oldBuffer, m_moveCount * sizeof( q3i32 ) );
		q3Free( oldBuffer );
	}
