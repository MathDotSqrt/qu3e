...
{
	const q3r32 k_fattener = q3r32( 0.5 );
...
//--------------------------------------------------------------------------------------------------
q3i32 q3DynamicAABBTree::Insert( const q3AABB& aabb, void *userData )
{
	q3i32 id = AllocateNode( );
...
void q3DynamicAABBTree::Remove( q3i32 id )
{
...
bool q3DynamicAABBTree::Update( q3i32 id, const q3AABB& aabb )
{
	assert( id >= 0 && id < m_capacity );
	assert( m_nodes[ id ].IsLeaf( ) );
...
void *q3DynamicAABBTree::GetUserData( q3i32 id ) const
{
	assert( id >= 0 && id < m_capacity );
...
const q3AABB& q3DynamicAABBTree::GetFatAABB( q3i32 id ) const
{
	assert( id >= 0 && id < m_capacity );
...
void q3DynamicAABBTree::RenderNode( q3Render *render, q3i32 index ) const
{
...
void q3DynamicAABBTree::ValidateStructure( q3i32 index ) const
{
	Node *n = m_nodes + index;
...
q3i32 q3DynamicAABBTree::AllocateNode( )
{
...
q3i32 q3DynamicAABBTree::Balance( q3i32 iA )
{
	Node *A = m_nodes + iA;
	...
	*/
	...
	q3i32 balance = C->height - B->height;
...
void q3DynamicAABBTree::InsertLeaf( q3i32 id )
{
	if ( m_root == Node::Null )
	{
		m_root = id;
	...
	while ( !m_nodes[ searchIndex ].IsLeaf( ) )
	{
		...
		q3r32 combinedArea = combined.SurfaceArea( );
		q3r32 branchCost = q3r32( 2.0 ) * combinedArea;

		// Inherited cost (surface area growth from heirarchy update after descent)
		q3r32 inheritedCost = q3r32( 2.0 ) * (combinedArea - m_nodes[ searchIndex ].aabb.SurfaceArea( ));

		q3i32 left = m_nodes[ searchIndex ].left;
		q3i32 right = m_nodes[ searchIndex ].right;
	...
	q3i32 sibling = searchIndex;

	// Create new parent
	q3i32 oldParent = m_nodes[sibling].parent;
	q3i32 newParent = AllocateNode( );
...
void q3DynamicAABBTree::RemoveLeaf( q3i32 id )
{
	if ( id == m_root )
	{
		m_root = Node::Null;
	...
	q3i32 parent = m_nodes[ id ].parent;
	q3i32 grandParent = m_nodes[parent].parent;
	q3i32 sibling;
...
void q3DynamicAABBTree::SyncHeirarchy( q3i32 index )
{
	while ( index != Node::Null )
	{
		index = Balance( index );

		q3i32 left = m_nodes[ index ].left;
		q3i32 right = m_nodes[ index ].right;
